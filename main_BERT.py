...
def clean_BERT_state_dict(state_dict):
    new_state_dict = OrderedDict()
    for k, v in state_dict.items():
        if k[:7] == 'module.':
            k = k[7:]  # remove `module.`
        if "bert" not in k and "feat_map" not in k:
            new_state_dict[k] = v
...
def main(args):
    ...
    if (not args.resume) and args.pretrain_model_path:
        ...
        _tmp_st = OrderedDict({k:v for k, v in clean_BERT_state_dict(checkpoint).items() if check_keep(k, _ignorekeywordlist)})
